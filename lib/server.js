...
exports.server=function(){
  ...
  if(typeof arguments[0] == "object" && arguments.length==1){
    ...
    envLocation=arguments[0];
    apiLocation=arguments[1];
    mongooseSchemaLocation=arguments[2];
    config=arguments[arguments.length-1];
    config.envLocation=envLocation;
    config.apiLocation=routesDirectory;
    config.mongooseSchemaLocation=mongooseSchemaLocation;
  } else {
  ...
  var redisUrl = url.parse(process.env.REDISTOGO_URL), redisAuth = redisUrl.auth.split(':');
