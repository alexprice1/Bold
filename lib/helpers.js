...
function initializeAPI(){
    ...
    for (var i in files) {
        ...
        var fileName = file.replace(serverConfig.api.location, "").replace(".js", "");
        var fns = require(rootUrl + "/" + files[i]);
        API[fileName] = {};
        for (var k in fns) {
            var fn = fns[k];
            if (typeof fn === "function") {
              createAPIFunction(fns[k], fileName, k);
            } else if(!serverConfig.api || !serverConfig.api.safeMade){
              API[fileName][k]=fn;
            }
        }
    }
...
function middleWare(args,count,data,fn,session,extras){
    if(count+1!=args.length){
        args[count](data,fn,session,extras,function(){
    ...
    if (!name || name != "API_CONSTRUCTOR") {
        ...
        fn = function() {
            try {
                ...
                console.log(clcError("Error: with API", serverConfig.apiLocation + fileName + "\n For function: " + exportName + "\n " + e.message));
            }
        };
    }
    API[fileName][exportName] = fn;
}
