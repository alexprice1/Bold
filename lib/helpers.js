...
exports.makeGlobal = function(userConfig, mongoose, io) {
  config = userConfig;
...
exports.attachAPI = function() {
  ...
  GLOBAL.API = function(route) {
    ...
    if (!methods) {
      try {
        methods = cwdRequire(apiLocation + route + '.js');
      } catch (err) {
        console.log(clcError('Error on API usage::', err));
        var err = 'Either this API does not exist, or you are requiring this API from a file that might currently being using this API';
        throw new Error(err);
...
function initializeAPI() {
  var files = getAllFilesFromDirectory(apiLocation);
  for (var i in files) {
    var file = files[i];
...
function initializeFilesAPIS(file, ext) {
  ...
  var fileName = file.replace(cwd + '/' + apiLocation, '').replace(apiLocation, '').replace('.js', '');
...
function getCurrentVersion() {
  if (config.main.APIversion) {
    return config.main.APIversion;
  }
  var dir = cwd + '/' + apiLocation;
  ...
  if (!(/[a-z]/i.test(params)) && (!name || name != 'API_CONSTRUCTOR')) {
    ...
    fn = function() {
      try {
        ...
        console.log(clcError('Error: with API', apiLocation + fileName + '\n For function: ' + exportName + '\n ' + e.message));
      }
    };
  }
