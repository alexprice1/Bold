...
function addSocketsToRoom(session,socket){
  if(!session || !socket){
    return;
  }
  ...
  var userNS = serverConfig.sessionUserNamespace;
  if(Array.isArray(userNS)){
    for(var i = 0; i< userNS.length; i++){
      var user = session[userNS[i]];
      if (user._id) {
        var userRoom = 'userId:' + user._id;
        if (user && user._id) {
          socket.join(userRoom);
          socket.join('onlineUsers');
          if (io.clients) {
            io.clients.setUserId(user._id, socket.id);
          }
        }
      }
    }
...
function apiSocket(socket, sessionStore) {
  ...
  socket.on('api', function(fileName, method, data, fn) {
    ...
    sessionStore.load(sessionID, function(storeErr, session) {
      ...
      addSocketsToRoom(session,socket);
